...
function updateGame() {
  ...
  for (let e of swarm) {
    ...
    if (dist(e.pos.x, e.pos.y, player.pos.x, player.pos.y) < 30) {
...
class Enemy {
  ...
  hunt(target) {
    let dir = p5.Vector.sub(target.pos, this.pos).setMag(1.2);
...
class Boss {
  ...
  hunt(target) {
    let dir = p5.Vector.sub(target.pos, this.pos).setMag(1);
    this.pos.add(dir);
  }
...
class Bullet {
  constructor(x, y, tx, ty) {
    ...
    this.vel = p5.Vector.sub(createVector(tx, ty), this.pos).setMag(10);
  }
